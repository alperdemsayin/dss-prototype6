...
def add_numbered_text_trace(
    ...
    fig.add_trace(
        go.Scattermapbox(
            ...
            mode="text",
            text=labels,
            textfont=dict(size=font_size + 8, color="#ffffff", family="Arial Black"),
...
def render_plant_map(active_rows: List[Dict], depot: Dict, plot_key: str) -> None:
    ...
    add_numbered_text_trace(
        fig,
        [depot["lat"]],
        [depot["lon"]],
        ["D"],
        [f"<b>{depot['name']}</b><br>Depot"],
        name="Depot",
        color="#2563eb",
        font_size=18,
        showlegend=True,
    ...
    if active_rows:
        ...
        add_numbered_text_trace(
            fig,
            [row["lat"] for row in active_rows],
            [row["lon"] for row in active_rows],
            [str(row["id"]) for row in active_rows],
            hover_rows,
            name="Plants",
            color="#059669",
            font_size=20,
...
def render_solution_map(result: Dict, active_rows: List[Dict], depot: Dict, rank: int) -> None:
    ...
    add_numbered_text_trace(
        ...
        [depot["lon"]],
        ["D"],
        [f"<b>{depot['name']}</b><br>Starting depot"],
        name="Depot",
        color="#2563eb",
        font_size=18,
        showlegend=True,
...
def render_one_solution(result: Dict, active_rows: List[Dict], depot: Dict, rank: int = 1) -> None:
    ...
    with tab1:
        info_panel(
            ...
            "No route lines are drawn. Blue D = depot. Large green numbers show the visit order clearly (1, 2, 3...). Gray numbers are active plants that were not visited in that solution.",
        )
...
def render_plant_map_page() -> None:
    ...
    with left:
        info_panel(
            "Map overview",
            "Blue D = Istanbul depot. Active plants are shown directly as large numbered labels so the plant IDs are easier to read on the map.",
        )
